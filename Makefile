...
VERSION ?= 0.0.1
IMAGE_TAG_BASE ?= devnation.redhat.com/greeting-operator
...
## marker
	cat target/kubernetes/greetings.devnation.redhat.com-v1.yml target/kubernetes/kubernetes.yml | operator-sdk generate bundle -q --overwrite --version 0.1.1 --default-channel=stable --channels=stable --package=greeting-operator
